...
@task
def deploy(service_name, port=2080):
    '''Deploy (or upgrade) YourTopia service named `service_name`'''
    basedir = '/home/okfn/var/srvc' 
    app_dir = basedir + '/' + service_name
    src_dir = app_dir + '/' + 'src'
    code_dir = src_dir + '/' + 'yourtopia'
    pip_path = app_dir + '/bin/pip'
    if not exists(src_dir):
        run('virtualenv %s' % app_dir)
        run('mkdir -p %s' % src_dir)
    run('%s install gunicorn' % pip_path)
    run('%s install -e git+https://github.com/okfn/yourtopia#egg=yourtopia' % pip_path)
...
command=%(app_dir)s/bin/gunicorn "yourtopia.web:app" --bind=127.0.0.1:%(port)s --workers=2 --max-requests=500 --name=%(service_name)s --error-logfile=%(log_path)s/%(service_name)s.gunicorn.error.log
